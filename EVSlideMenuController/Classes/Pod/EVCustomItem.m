...
@implementation EVCustomItemModel

+ (instancetype)EVCustomItemModelWithNorImageName:(NSString *)norImageName
                                   selImageName:(NSString *)selImageName
                                          title:(NSString *)title {
    EVCustomItemModel *itemModel = [[self alloc] init];
    itemModel.norImageName = norImageName;
    itemModel.selImageName = selImageName;
...
+ (instancetype)buttonWithType:(UIButtonType)buttonType {
    EVCustomItem *item = [super buttonWithType:buttonType];
    [item setup];
    return item;
}
...
- (void)setup {
    ...
    _badgeStyle = EVCustomItemBadgeStyleNumber;
    self.badge = 0;
}

/**
 *  覆盖父类的setHighlighted:方法，按下EVCustomItem时，不高亮该item
 */
...
- (void)setContentHorizontalCenterWithVerticalOffset:(CGFloat)verticalOffset
    ...
    self.titleAndImageMargin = titleAndImageMargin;
...
- (void)layoutSubviews {
    ...
    if ([self imageForState:UIControlStateNormal] && self.contentHorizontalCenter) {
        ...
        self.titleEdgeInsets = UIEdgeInsetsMake(self.verticalOffset+self.titleAndImageMargin, - imageSize.width, - (totalHeight - titleSize.height), 0);
    } else {
...
- (void)setBadgeStyle:(EVCustomItemBadgeStyle)badgeStyle {
    _badgeStyle = badgeStyle;
    [self updateBadge];
}

- (void)updateBadge {
    if (self.badgeStyle == EVCustomItemBadgeStyleNumber) {
        if (self.badge == 0) {
    ...
    } else if (self.badgeStyle == EVCustomItemBadgeStyleDot) {
